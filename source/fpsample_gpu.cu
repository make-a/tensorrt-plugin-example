...
__device__ void __update( float* __restrict__ dists, int64_t* __restrict__ dists_i, int idx1, int idx2 ) {
...
template <unsigned int block_size>
__global__ void furthest_point_sampling_kernel( int b, int n, int m, const float* __restrict__ dataset,
                                                float* __restrict__ temp, int64_t* __restrict__ idxs ) {
    if ( m <= 0 )
        return;
    __shared__ float   dists[ block_size ];
    __shared__ int64_t dists_i[ block_size ];
...
// void furthest_point_sampling_kernel_wrapper( int b, int n, int m, const float* dataset, float* temp, int* idxs,
//                                              cudaStream_t stream ) {
